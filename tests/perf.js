...
// and fewer particles; 2 = also no floor reflections; 3 = also no bloom or beam dust; 4 = also the match at 1.25× pixel
// ratio), at 1× and 4× CPU throttling (about a mid-range phone). Then real frames run for a few seconds at 4× to check that the guard engages by itself.
...
(async () => {
  ...
  for (const throttle of [1, 4]) {
    ...
    if (throttle > 1) { // real frames: release the pin and let the page run for a while
      await P.ev(() => { const g = HH.game; g.guard.force = null; g.guard.lock = false; g.guard.level = 0; g.guard.slowT = 0; }); await P.page.waitForTimeout(12000);
