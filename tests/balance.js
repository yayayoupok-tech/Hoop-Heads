...
// Legend vs Pro plays 8 games per cell count (at least 60): 36 games left a ±7% standard error on a 75–95% target.
...
(async () => {
  ...
  const lvp = await P.ev(n => { const a = runSims({ n, mode: '1v1', difficulty: 'legend', difficulty2: 'pro', format: { type: 'first', target: 21 }, teamA: 'legends', teamB: 'legends' }); return { wins: a.wins, games: a.games, ppp: [a.teams[0].ppp, a.teams[1].ppp] }; }, Math.max(60, n * 8));
